...
import { adminTodos, markCompleted, deleteTodo, updateTodo, getAllUsers, changeUserStatus, getCompletedTodos, getNonUrgentTodos, getUrgentTodos, removeUser } from '../setUp/todosThunk.js'
        ...
        <div className='flex gap-8'>
          ...
          <div className='w-[150px] min-h-[670px] rounded-md dark:bg-gray-600'>
            <ul className='align-middle'>
              ...
              <li><button className='hover:bg-slate-400 p-3 rounded-xl ml-3' onClick={() => gettingUsers()}>All Users </button></li>
              ...
              <div className='modal-box w-[500px]'>
                <form onSubmit={handleSubmit(handleUpdate)}>
                  ...
                  <div>
                    ...
                    <textarea id='desc' className="textarea h-24 w-[400px] focus: outline-none border dark:border-white p-1"
                      {...register("description")}
                    />
                  </div>
                  <button type='submit' className='btn dark:bg-green-300 dark:text-black w-[100px] font-bold mt-3' onClick={() => document.getElementById("my_modal_1").close()}>Update</button>
                </form>
              </div>
            </dialog>
            {
              todos.map((todo) =>
                ...
                <div className={`card ${todo.completed ? "bg-green-300 text-black" : "bg-primary  text-primary-content"}   w-96 h-[150px] m-4`} key={todo._id}>
                    ...
                    <div className='relative z-10 top-[130px] left-[160px]'>
                      {
                        user.role === "admin" ?
                          ...
                          <button className='z-10 btn p-2 bg-yellow-200 text-black font-bold' onClick={() => changeStatus(user._id)}>
                            Demote to user
                          </button>
                          : <button className='z-10 btn p-2 bg-yellow-200 text-black font-bold' onClick={() => changeStatus(user._id)}>
                            Promote to admin
                     ...
                     <svg viewBox="-0.5 0 19 19" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" 
                     onClick={()=> removeMember(user._id)}
                     xmlns:sketch="http://www.bohemiancoding.com/sketch/ns" fill="#000000" className='w-[25px] z-10 relative top-[350px] left-[270px] text-white'><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>icon/18/icon-delete</title> <desc>Created with Sketch.</desc> <defs> </defs> <g id="out" stroke="none" strokeWidth="1" fill="0" fillRule="evenodd" sketch:type="MSPage"> <path d="M4.91666667,14.8888889 C4.91666667,15.3571429 5.60416667,16 6.0625,16 L12.9375,16 C13.3958333,16 14.0833333,15.3571429 14.0833333,14.8888889 L14.0833333,6 L4.91666667,6 L4.91666667,14.8888889 L4.91666667,14.8888889 L4.91666667,14.8888889 Z M15,3.46500003 L12.5555556,3.46500003 L11.3333333,2 L7.66666667,2 L6.44444444,3.46500003 L4,3.46500003 L4,4.93000007 L15,4.93000007 L15,3.46500003 L15,3.46500003 L15,3.46500003 Z" id="path" fill="white" sketch:type="MSShapeGroup"> </path> </g> </g></svg>
                    <div className='relative z-10 top-[80px]'>
                      <div><span className='font-bold m-3'>Name:</span>{user.username}</div>
                      <div><span className='font-bold m-3 mr-5'>Role:</span>{user.role}</div>
