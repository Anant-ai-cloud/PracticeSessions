          ...
          todos.map((todo) =>
            ...
            <div className={`card ${todo.completed ? "bg-green-300 text-black" : "bg-primary  text-primary-content"}   w-96 m-4`} key={todo._id}>
              <div className="card-body">
                ...
                onClick={()=> {document.getElementById("updatemodal").showModal(), setTitle(todo.title), setDescription(todo.description)}}
                ...
                <dialog id='updatemodal' className='modal'>
                  ...
                  <form  onSubmit={(e)=>{handleSubmit(e,{title: title, description: description}, todo._id), setTitle(""), setDescription("")}}>
                    <div className='modal-box border border-white bg-[#1E1E1E] p-5 w-[350px]'>
                    <button type='button' className='relative left-[290px] text-white p-2 rounded-md' onClick={()=>{ document.getElementById("updatemodal").close(), setTitle(""), setDescription("")}}>X</button>
                    <div>
                      <label htmlFor="title" className='text-[#F1F5F9] font-semibold'>Updated Title</label> <br />
                      <input id='title' value={todo.title} type="text" className='input w-[290px] mt-2 mb-2 text-white focus: outline-none border border-white bg-[#1E293B]'
                      {...register("title",{
                        required:true
                      })}
                      />
                    ...
                    <div>
                      <label htmlFor="description" className='text-white font-semibold'>Updated Description</label> <br />
                      <textarea  id="description" value={todo.description} className='w-[290px] h-[100px] text-white mt-2 focus: outline-none border border-white bg-[#1E293B]' {...register("description")}/>
                        
                    </div>
                       <button type='submit' className="btn bg-[#1A77F2] text-white border-[#005fd8] w-[200px] ml-[40px] rounded-md mt-2" onClick={()=> document.getElementById("updatemodal").close()}> Update</button>
                       </div>
                  </form>
