            ...
            <div className={`card ${todo.completed ? "bg-green-300 text-black" : "bg-primary  text-primary-content"}   w-96 m-4`} key={todo._id}>
              <div className="card-body">
                ...
                <dialog id='updatemodal' className='modal'>
                  ...
                  <form  onSubmit={(e)=> handleSubmit(e,{title: title, description: description}, todo._id)}>
                    <div className='modal-box border border-white bg-[#1E1E1E] p-5 w-[350px]'>
                    <button type='button' className='relative left-[290px] text-white p-2 rounded-md' onClick={()=> document.getElementById("updatemodal").close()}>X</button>
                    <div>
                      <label htmlFor="title" className='text-[#F1F5F9] font-semibold'>Updated Title</label> <br />
                      {console.log(todo.title)}
                      <input id="title" key={todo._id} value= {todo.title} type="text" className='input w-[290px] mt-2 mb-2 text-white focus: outline-none border border-white bg-[#1E293B]'
                      {...register("title",{
                        required:true
                      })}
                      />
